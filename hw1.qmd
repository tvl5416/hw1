...
The vector contains strings with representing numeric value.
...
2.  Create a new vector `my_vec_bool` which comprises of:
    ...
    There are 4 elements that are greater than zero.

3.  Sort the values of `my_vec_double` in ascending order.
...
```{R}
#Second matric
matrix(c(1:100, (1:100)^2), nrow=2, byrow= TRUE)
```
...
col_wise_scan is expected to run shorter because R stores and accesses
matrices in column-major order, which aligns with how matrices are
stored in memory.
...
list(
    row_wise_time = time_scan(row_wise_scan, M1),
    col_wise_time = time_scan(row_wise_scan, M1)
)
```
...
list(
    row_wise_time = time_scan(row_wise_scan, M2),
    col_wise_time = time_scan(row_wise_scan, M2)
)
```
...
list(
    row_wise_time = time_scan(row_wise_scan, M3),
    col_wise_time = time_scan(row_wise_scan, M3)
)
```
...
I can conclude that col_wise_scan likely performs faster than row_wise_scan for large matrices, due to R's column-major order of matrix storage, which allows more efficient memory access during column-wise operations.
